  ...
  try {
    ...
    if (user.role === "owner") {
      restaurant = await Restaurant.findOne({ _id: id, user: user }).populate({
        path: "reviews",
        populate: [
          { path: "from_user", select: "email firstName lastName" },
          { path: "restaurant", select: "name overall_rating highest_rating lowest_rating" }
        ]
      });
    } else {
      restaurant = await Restaurant.findOne({ _id: id }).populate({
        path: "reviews",
        populate: [
          { path: "from_user", select: "email firstName lastName" },
          { path: "restaurant", select: "name overall_rating highest_rating lowest_rating" }
    ...
    res.send({
      ...
      reviews: restaurant.reviews
        ...
        .slice(
          ...
          parseInt(page - 1) * parseInt(pageSize) + parseInt(pageSize)
        ),
  ...
  try {
    ...
    if (user.role === "regular" || user.role === "admin") {
      return res.status(403).send({
        message: "You're not authorized to get a pending review list."
      });
    }
    ...
    res.send({
      ...
      count: reviewArray.filter(a => a.reply === "").length
    });
  } catch (err) {
    next(err);
  }
...
async function create(req, res, next) {
  try {
    const { rate = 0, comment, reply, date } = req.body;
    const { id } = req.params;
    const { user } = req;
...
async function update(req, res, next) {
  const { id } = req.params;
  const { reply, comment, rate, date } = req.body;
  const user = req.user;
  ...
  if (_.isUndefined(date) && user.role === "admin") {
    return res.status(400).send({ message: "Date is required in admin view" });
  }
